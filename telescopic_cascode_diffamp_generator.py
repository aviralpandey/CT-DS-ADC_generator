...
# Define the parameter class we will use to feed MOS generators later on
@h.paramclass
class MosParams:
...
# Define the sky130 MOSFETs as external modules (i.e. not primitives) for HDL21
...
def stage1_telescopic_amplifier_design_input(database,amp_specs,gen_params):
    ...
    for vds_in in vds_in_val_list:
        ...
        try:
            vbs_in = vb - vsource
            vgs_in = query_db_for_vgs(database, in_type, lch_in, vbs_in, vds_in, vstar=vstar_in)
        except ValueError:
            continue 
...
def AmplifierTbTran(params: Stage1TbParams) -> h.Module:
    ...
    tb.inpgen = DiffClkGen(
        DiffClkParams(period= 1000 * n, delay=1 * n, vc=params.vc, vd=params.vd, trf=800 * PICO)
...
def generate_stage1_telescopic_amplifier(amp_specs):
    ...
    params = Stage1TbParams(pvt=Pvt(), vc=amp_specs['vincm'] * 1000 * m, vd=1 * m, dut=ampParams, cl = amp_specs['cload'] * 1000 * m)
...
def stage2_common_source_amplifier_design_and_scale_amp(database_in,database_load,amp_specs,gen_params):
    ...
    load_scale_list = np.linspace(load_scale_min,load_scale_max,load_scale_count,endpoint=True)
    ...
    for lch_load in gen_params['lch_load']:
        ...
        for load_scale in load_scale_list:
            ...
            try:
                vgs_load = query_db_for_vgs(database_load,load_type,lch_load,vbs_load,vds_load,ids=ids_load,mode='ids',scale=1)
            except:
                continue
            
            gm_load = query_db(database_load,'gm',load_type,lch_load,vbs_load,vgs_load,vds_load)*load_scale
            gds_load = query_db(database_load,'gds',load_type,lch_load,vbs_load,vgs_load,vds_load)*load_scale
            cgg_load = query_db(database_load,'cgg',load_type,lch_load,vbs_load,vgs_load,vds_load)*load_scale
    ...
    scale_bw = max(1, 2 * np.pi * gbw_min * cload / (gm_in - 2 * np.pi * gbw_min * (cdd+cgg_in)))
    ...
    stage2AmpParams=Stage2AmpParams(
        in_params = MosParams(w= 500 * m * int(np.round(amplifier_op['scale_in'])), l=gen_params['lch_in'], nf=int(np.round(amplifier_op['scale_in']))),
        load_params = MosParams(w= 500 * m * int(np.round(amplifier_op['scale_load'])), l=amplifier_op['lch_load'], nf=int(np.round(amplifier_op['scale_load']))),
        in_type = amp_specs['in_type'],
        load_type = amp_specs['load_type'],
    ...
    params = Stage2AmpTbParams(pvt=Pvt(), vc=vincm * 1000 * m, vd=1 * m, dut=stage2AmpParams, cl = amp_specs['cload'] * 1000 * m)
...
def generate_stage2_common_source_amplifier(amp_specs):
    ...
    database_in = np.load(in_db_filename,allow_pickle=True).item()
    database_load = np.load(load_db_filename,allow_pickle=True).item()
    ampParams, amplifier_op = stage2_common_source_amplifier_design_and_scale_amp(database_in,database_load,amp_specs,gen_params)
...
class TwoStageAmpParams:
    ...
    stage1_in_base_pair_params = h.Param(dtype=MosParams, desc="Stage 1 Input pair MOS params")
    stage1_in_casc_pair_params = h.Param(dtype=MosParams, desc="Stage 1 Input cascode MOS params")
    stage1_load_base_pair_params = h.Param(dtype=MosParams, desc="Stage 1 Load base MOS params")
    stage1_load_casc_pair_params = h.Param(dtype=MosParams, desc="Stage 1 Load cascode MOS params")
...
def two_stage_amplifier(params: TwoStageAmpParams) -> h.Module:
    ...
    class TwoStageAmplifier:
        ...
        ## Stage 2 Input Devices
        m_in_stage2_p = stage2_mos_in(params.stage2_in_params)(g=v_out_stage1.n, s=VSS, d=v_out.p, b=VSS)
        m_in_stage2_n = stage2_mos_in(params.stage2_in_params)(g=v_out_stage1.p, s=VSS, d=v_out.n, b=VSS)
        
        ## Load Base Pair
        m_load_stage2_p = stage2_mos_load(params.stage2_load_params)(g=v_load_stage2, s=VDD, d=v_out.p, b=VDD)
        m_load_stage2_n = stage2_mos_load(params.stage2_load_params)(g=v_load_stage2, s=VDD, d=v_out.n, b=VDD)
...
def TwoStageAmpTbTran(params: TwoStageTbParams) -> h.Module:
    ...
    tb.v_tail_stage1_src = Vdc(Vdc.Params(dc=(params.dut.stage1_v_tail),ac=(0*m)))(p=tb.v_tail_stage1, n=tb.VSS)
    
    tb.v_pcasc_stage1 = h.Signal()
    tb.v_pcasc_stage1_src = Vdc(Vdc.Params(dc=(params.dut.stage1_v_pcasc),ac=(0*m)))(p=tb.v_pcasc_stage1, n=tb.VSS)
    
    tb.v_ncasc_stage1 = h.Signal()
    tb.v_ncasc_stage1_src = Vdc(Vdc.Params(dc=(params.dut.stage1_v_ncasc),ac=(0*m)))(p=tb.v_ncasc_stage1, n=tb.VSS)
    ...
    tb.v_outcm_stage1_ideal_src = Vdc(Vdc.Params(dc=(params.dut.stage1_voutcm_ideal),ac=(0*m)))(p=tb.voutcm_stage1_ideal, n=tb.VSS)
    
    tb.voutcm_stage2_ideal = h.Signal()
    tb.v_outcm_stage2_ideal_src = Vdc(Vdc.Params(dc=(params.dut.stage2_voutcm_ideal),ac=(0*m)))(p=tb.voutcm_stage2_ideal, n=tb.VSS)
...
def generate_two_stage_ota():
    ...
    stage2_common_source_amp_specs = dict(
        ...
        vnoise_input_referred=two_stage_amp_specs['vnoise_input_referred'] * (telescopic_amp_results['Av']/(telescopic_amp_results['Av'] + two_stage_amp_specs['gain_min'])) * telescopic_amp_results['Av'],
        )
    
    stage2_params, stage2_op = generate_stage2_common_source_amplifier(stage2_common_source_amp_specs)
    ...
    params = TwoStageTbParams(pvt=Pvt(), vc=two_stage_amp_specs['stage1_vincm'] * 1000 * m, vd=1 * m, dut=twoStageAmpParams, cl = two_stage_amp_specs['cload'] * 1000 * m)
    ...
    v_out_stage1_diff = tran_results['v(xtop.out_stage1_p)'] - tran_results['v(xtop.out_stage1_n)']
    v_out_stage1_cm = (tran_results['v(xtop.out_stage1_p)'] + tran_results['v(xtop.out_stage1_n)'])/2
