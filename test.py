...
def get_amp_performance(amp: h.Module) -> Tuple[float, float]:
    ...
    tb.vout = h.Signal()
    tb.vin = h.Signal()
    tb.VDD_src = Vdc(Vdc.Params(dc=1800 * m))(p=tb.VDD, n=tb.VSS)
    tb.vin_src = Vdc(Vdc.Params(dc=1500 * m, ac=1 * UNIT))(p=tb.vin, n=tb.VSS)
    tb.dut = amp(VDD=tb.VDD, VSS=tb.VSS, vin=tb.vin, vout=tb.vout)

    sim = h.sim.Sim(tb=tb)
    sim.lib(
        f"{CONDA_PREFIX}/share/pdk/sky130A/libs.tech/ngspice/sky130.lib.spice", "tt"
    )
